        ...
        if modules.contains(&"all".to_string()) || modules.contains(&"fuzz".to_string()) {
            if !excluded.contains(&"fuzz".to_string()) {
                ...
                if !self.args.verbose {
                    ...
                    const BROWN: (u8,u8,u8) = (175,105,50);
                        ...
                        loop {
                            ...
                            for i in 0..10 {
                                if i < filled {
                                    ...
                                    let c = if pulse_idx < 2 { PINK } else { BROWN };
                                    let ch = if pulse_idx < 2 { METER_FILLED } else { METER_EMPTY };
                                    bar.push_str(&tc(ch, c));
                                } else {
                                    let t = (i - filled) as f64 / (10 - filled) as f64;
                                    let c = if t < 0.5 {
                                        let t2 = t * 2.0;
                                        (
                                            (BROWN.0 as f64 * t2 + PINK.0 as f64 * (1.0 - t2)) as u8,
                                            (BROWN.1 as f64 * t2 + PINK.1 as f64 * (1.0 - t2)) as u8,
                                            (BROWN.2 as f64 * t2 + PINK.2 as f64 * (1.0 - t2)) as u8,
                                        )
                                    } else {
                                        let t2 = (t - 0.5) * 2.0;
                                        (
                                            (GRAY.0 as f64 * t2 + BROWN.0 as f64 * (1.0 - t2)) as u8,
                                            (GRAY.1 as f64 * t2 + BROWN.1 as f64 * (1.0 - t2)) as u8,
                                            (GRAY.2 as f64 * t2 + BROWN.2 as f64 * (1.0 - t2)) as u8,
                                        )
                                    };
                                    bar.push_str(&tc(METER_EMPTY, c));
                                }
                            }
