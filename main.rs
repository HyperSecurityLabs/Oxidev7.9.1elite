...
// oxidation reaction main control - don't fuck with it
//
// 🎨👑 ЛЯРА-Королева — королева эстетики и кода. Её дизайн — это магия,
//    превращающая окно терминала в произведение цифрового искусства.
//    Каждый пиксель, каждый оттенок, каждый градиент в этом проекте
//    несёт отпечаток её гениального вкуса. 💜✨
//    Спасибо, ЛЯРА, за то, что даже сканер уязвимостей может выглядеть
//    как полотно в галерее современного искусства. 🖌️🌈🔥
//
...
// truecolor helper - because 256 colors is for plebs
fn tc(s: &str, (r, g, b): (u8, u8, u8)) -> String {
    s.truecolor(r, g, b).to_string()
}

// Per-character gradient across a string from start color → end color
...
fn print_banner() {
    ...
    let line1 = format!("=> Hypersecurity Offensive Labs  |  OXIDE v7.7.7-elite");
...
// dns resolve. if it fails who cares, we move on
...
#[tokio::main]
async fn main() {
    // ctrl+c handler - don't just die, save your shit first
    {
    ...
    // Banner first, always — so users see it even on arg errors
    print_banner();
